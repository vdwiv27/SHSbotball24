...
class CameraHandler:
      ...
      if area_threshold != None:
          ...
          while kipr.get_object_confidence(channel, obj) < 0.25:
              ...
              if timer and time.time() - start > 3:
                  ...
                  return -1
      print("Area:", kipr.get_object_area(channel, obj))
      print("X:", kipr.get_object_center_x(channel, obj))
      print("found object")
      return 0
    ...
    def align_with_object_x(self, channel, obj, center, max_deviation=5, turn_speed=1, timer=True):
      ...
      while abs(obj_x - center) > max_deviation:
        speed = 25
        if obj_x < center:
          self.drive.drive_speed({0: speed, 1:-speed})
          print("ayaan is stupid")
        else:
          self.drive.drive_speed({0:-speed, 1:speed})
          print("Shash is a little overweight")
        ...
        # 10 Second timeout
        if timer and time.time() - start > 20:
            print("stopped aligning early")
            return -1

      print("Final Obj_x", obj_x)
      print("aligned")
#drive_towards_objects(self, "green.conf", 1, get_objects_x_center("green",1),get_objects_y_center("green",1))
    def drive_towards_object(self, channel, obj, dest_x, dest_y, area_threshold=None, max_deviation=100, turn_speed=1, direction=1, timer=True):
      ...
      while abs(obj_y - dest_y) > max_deviation:
        ...
        if obj_y < dest_y:
          self.drive.drive_speed({0:250, 1:250})
          #Ayaan is stupid
        else:
          self.drive.drive_speed({0:-250, 1:-250})
        kipr.camera_update()
    ...
    def drive_towards_object_pom(self, channel, obj, dest_x, dest_y, area_threshold=None, max_deviation=5, turn_speed=1, direction=1, timer=True):
      ...
      while abs(obj_y - dest_y) > max_deviation:
        ...
        if deviation < 0.4 * dest_y and deviation > 0.25 * dest_y:
          ...
          self.drive.drive_speed(AVERAGE_MOTOR_SPEED, AVERAGE_MOTOR_SPEED)
        else:
          self.drive.drive_speed(-AVERAGE_MOTOR_SPEED, -AVERAGE_MOTOR_SPEED)
        kipr.camera_update()
        obj_y =  kipr.get_object_center_y(channel, obj)
