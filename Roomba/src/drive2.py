...
class Drive2:
  ...
  def calc_angle_speed_ramp(self, final_angle, current_angle, accel, decel, adjustment=False):
    ...
    if accel and (current_angle < accel_angle):
      norm_x = current_angle / accel_angle
      tween = abs(math.sin(norm_x * math.pi / 2))
      if (tween < 0.2):
        tween = 0.2
    
    elif decel and (current_angle > final_angle - accel_angle):
  ...
  def calc_speed_ramp(self, final_dist, current_dist, accel, decel):
    if final_dist < 20:
       accel_distance = min(5, final_dist / 2)
    else:
       accel_distance = 10

    if accel and (current_dist < accel_distance):
      norm_x = current_dist / accel_distance
      tween = math.sin(norm_x * math.pi / 2)
      if (tween < 0.2):
        tween = 0.2
    
    elif decel and (current_dist > final_dist - accel_distance):
  ...
  def drive_distance_y(self, distance, base_speed, debug=False, accel=False, decel=True):
    # kipr.set_create_distance(0)
    ...
    if abs(base_speed) == 250:
        KP_drive = 5
    elif abs(base_speed) == 450:
        KP_drive = 2.5
    else:
        print("PID NOT TUNED FOR THIS BASE SPEED... PROCEED WITH CAUTION!", base_speed)
        KP_drive = 5
    ...
    while (dist_traveled < distance):
      mult = self.calc_speed_ramp(distance, dist_traveled, accel, decel)
      if self.angle_target % 360 == 0:
          error = self.yPose - start_y
      else:
          error = start_y - self.yPose
  ...
  def drive_distance(self, distance, base_speed, debug=False, accel=False, decel=True):
    # kipr.set_create_distance(0)
    # dist = kipr.get_create_distance()
  ...
  def drive_distance_x(self, distance, base_speed, debug=False, accel=False, decel=True):
    # kipr.set_create_distance(0)
    # dist = kipr.get_create_distance()
    ...
    while (dist_traveled < distance):
      mult = self.calc_speed_ramp(distance, dist_traveled, accel, decel)
      if (self.angle_target - 90) % 360 == 0:
          error = start_x - self.xPose
      else:
  ...
  def turn2(self, degrees, speed, debug=False, accel=False, decel=True):
    ...
    while abs(math.degrees(self.thrPose) - self.angle_target) > 0.25:
       mult, adjusting = self.calc_angle_speed_ramp(degrees, angle_traveled, accel, decel, adjusting)
    ...
    self.lock_wheels()
    print("Final", mult, abs(math.degrees(self.thrPose) - self.angle_target), angle_traveled, self.leftEncoder, self.rightEncoder, -left_motor_dir * mult * speed * error/abs(error) * -degrees/abs(degrees))

  def turn(self, degrees, speed, debug=False, accel=False, decel=True):
    ...
    while abs(angle_traveled - degrees) > 0.25:
       mult, adjusting = self.calc_angle_speed_ramp(degrees, angle_traveled, accel, decel, adjusting)
