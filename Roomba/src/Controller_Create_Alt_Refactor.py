...
class Controller:
    ...
    def execute_game(self):
        ...
        self.utilities.align_with_line(direction=-1,cross_line=True)
        self.utilities.follow_line_PID(KP, KI, KD, LINE_FOLLOW_BASE_SPEED, side="right", location="floor", time_to_travel=2.5, direction=1)
       ...
       # self.recenter_create()
        ...
        #self.drive_to_botgal_alt()
        #self.grab_and_drop_botgal_alt()
    ...
    def drive_to_botgal(self):            
        ...
        t.join()
        self.drive.drive_distance(33, -250)
        self.drive_till_object_detected(-150)
    ...
    def drive_to_botgal_alt(self):
        ...
        t.start()
        self.drive.drive_distance(90, -450)
    
    def grab_and_drop_botgal_alt(self):
        # Raise botgal
        print("GRABBING BOTGAL")
        self.servos.change_servo_positions(SERVO_INCREMENT_FAST, SERVO_SLEEP_TIME, {CLAW_PORT: CLAW_CLOSED_POSITION})
        self.servos.change_servo_positions(SERVO_INCREMENT_FAST, SERVO_SLEEP_TIME, {ARM_PORT: ARM_BOTGAL_RAISE_POSITION, WRIST_PORT: WRIST_BOTGAL_RAISE_POSITION})
        time.sleep(0.25)
        self.drive.drive_distance(10, 250)
        self.drive.turn(30, 250)
        ...
        self.drive.drive_distance(20, 250)
        t.start()
        self.utilities.align_with_line(direction=1,cross_line=False)
        self.drive.turn(135, 250)
        t.join()
            
        self.servos.change_servo_positions(SERVO_INCREMENT_FAST, SERVO_SLEEP_TIME, {ARM_PORT: ARM_BOTGAL_DROP_POSITION, WRIST_PORT: WRIST_BOTGAL_DROP_POSITION, CLAW_PORT: CLAW_OPEN_POSITION})            
        time.sleep(0.25)
    ...
    def recenter_create(self):
        ...
        # Alignment steps
        self.drive.drive_distance(15, -250)
    ...
    def drive_to_yellow_cube_left_alt(self):   
        ...
        self.drive.drive_distance(LEFT_TALL_TOWER_HORIZONTAL_DISTANCE, -250)
        ...
        self.drive.drive_distance(LEFT_TALL_TOWER_VERTICAL_DISTANCE, -250)
        self.drive_till_object_detected(-150)
        t.join()
    ...
    def drive_back_to_lab(self, side, height, first=False):
        ...
        # Might need to lower claw simultaneously to avoid tipping the robot
        # and risking the reflectance sensors not seeing the black line

        if height == "short":
            self.drive.drive_distance(50, 250 * direction)
        else:
            self.drive.drive_distance(15, 250 * direction)
        ...
        self.drive.drive_distance(30, -250)
    ...
    def drive_to_cube(self, vertical_distance_alignment, horizontal_distance_alignment, vertical_distance_final, side, height):
        ...
        self.drive.drive_distance(vertical_distance_alignment, 250)
        t.join()
        ...
        self.drive.drive_distance(horizontal_distance_alignment, -250)
        # face yellow cube
        self.drive.turn(90 * direction, 250)
        # alignment steps
        #self.drive.drive_distance(5, -250)
        ...
        if side == "left" and height == "short":
            t.join()
            self.drive.drive_distance(vertical_distance_final, -250)
            self.drive_till_object_detected(-150)
    ...
    def stack_cube(self, backup_distance, arm_position, wrist_position, last=False):
        ...
        self.drive.drive_distance(backup_distance, 250)
    ...
    def drive_till_object_detected(self, speed):
        val = 0
        start = time.time()
        while val < 50:
            if self.sensors[RANGEFINDER].analog() > 2850:
                val += 1
            if time.time() - start > 2:
                print("TIMEOUT DURING RANGEFINDER OBJECT DETECTION")
                break
            self.drive.drive_speed(speed, speed)
        print("OBJECT DETECTED")
        self.drive.drive_distance(2, speed)
        self.drive.lock_wheels()
